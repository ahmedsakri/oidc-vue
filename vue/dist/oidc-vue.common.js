...
function createOidcAuth(authName, defaultSignInType, appUrl, oidcConfig, logger, logLevel) {
  ...
  external_oidc_client_["Log"].debug(`Creating new oidc auth as ${authName}`);
  const mgr = new external_oidc_client_["UserManager"](config);
  let _inited = false;
  const auth = new external_commonjs_vue_commonjs2_vue_root_Vue_default.a({
    data() {
      return {
        mgr,
        user: null,
    ...
    methods: {
      startup() {
        ...
        if (matchesPath(config.popup_redirect_uri)) {
          external_oidc_client_["Log"].debug(`${authName} Popup signin callback`);
          mgr.signinPopupCallback();
          isCB = true;
        } else if (matchesPath(config.silent_redirect_uri)) {
          external_oidc_client_["Log"].debug(`${authName} Silent signin callback`);
          mgr.signinSilentCallback();
          isCB = true;
        } else if (matchesPath(config.popup_post_logout_redirect_uri)) {
          external_oidc_client_["Log"].debug(`${authName} Popup logout callback`);
        ...
        if (_inited) {
          ...
          return mgr.getUser().then(test => {
            ...
            external_oidc_client_["Log"].warn(`Auth startup err = ${err}`);
            return false;
          });
        }
      },
      useRouter(router) {
        ...
        router.beforeEach((to, from, next) => {
          if (to.matched.some(record => record.meta.authName === this.authName)) {
            if (this.isAuthenticated) {
              external_oidc_client_["Log"].debug(`${authName} auth authenticated user entering protected route ${to.fullPath}`);
              next();
            } else {
              external_oidc_client_["Log"].debug(`${authName} auth anon user entering protected route ${to.fullPath}`);
        ...
        if (config.redirect_uri) {
          const vroutePath = '/' + getUrlPath(config.redirect_uri).substring((router.options.base || '/').length);
          router.addRoute({
            path: vroutePath,
            ...
            component: {
              ...
              created() {
                mgr.signinRedirectCallback().then(data => {
                  external_oidc_client_["Log"].debug(`${authName} Window signin callback success`, data);
                  ...
                  external_oidc_client_["Log"].error(`${authName} Window signin callback error`, err);
  ...
  function signInIfNecessary() {
    if (auth.myRouter) {
      ...
      if (current && current.meta && current.meta.authName === authName) {
        external_oidc_client_["Log"].debug(`${authName} auth page re-signin with ${defaultSignInType}`);
        signInReal(defaultSignInType, {
          state: {
            current
          }
  ...
  function handleManagerEvents() {
    ...
    mgr.events.addUserUnloaded(() => {
      ...
      external_oidc_client_["Log"].debug(`${auth.authName} auth user unloaded`);
      // signInIfNecessary()
    });

    mgr.events.addAccessTokenExpired(() => {
      external_oidc_client_["Log"].debug(`${auth.authName} auth token expired, user is authenticated=${auth.isAuthenticated}`);
      auth.user = null;
      signInIfNecessary();
    ...
    mgr.events.addSilentRenewError(e => {
      external_oidc_client_["Log"].debug(`${auth.authName} auth silent renew error ${e}`);
      // TODO: need to restart renew manually?
      if (auth.isAuthenticated) {
        setTimeout(() => {
          external_oidc_client_["Log"].debug(`${auth.authName} auth silent renew retry`);
          mgr.signinSilent();
        }, 5000);
      } else {
        signInIfNecessary();
    ...
    mgr.events.addUserSignedOut(() => {
      external_oidc_client_["Log"].debug(`${auth.authName} auth user signed out`);
      auth.user = null;
      signInIfNecessary();
    });
  }
